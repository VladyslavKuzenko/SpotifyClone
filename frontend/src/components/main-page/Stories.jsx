...
const Stories = () => {
  ...
  const goToNextUser = () => {
    if (currentUserIndex === -1) {
      // після тебе — перший з uniqueUsers
      if (uniqueUsers.length > 0) {
        setCurrentUserIndex(0);
        const group = groupedStories[uniqueUsers[0].user.id];
      ...
      const newUserIndex = currentUserIndex + 1;
      setCurrentUserIndex(newUserIndex);
      const group = groupedStories[uniqueUsers[newUserIndex].user.id];
      setCurrentStoryGroup(group);
      setCurrentStoryIndex(0);
    }
  };
  ...
  const goToPrevUser = (openLastStory = false) => {
    if (currentUserIndex === -1) return; // ти був першим, нічого лівіше
    if (currentUserIndex === 0 && hasMyStories) {
      setCurrentUserIndex(-1);
      setCurrentStoryGroup(myStories);
      setCurrentStoryIndex(openLastStory ? myStories.length - 1 : 0);
  ...
  //______________________________________________________________________________
  const userHasStoriesBefore = () => {
    // Якщо в тебе є сторіс, але твій юзер не в списку, то вона перед поточним
    const myStories = groupedStories[user?.sub];
    const myIndex = uniqueUsers.findIndex((u) => u.user.id === user?.sub);

    // Якщо user є в списку, але не перший
    if (myIndex > 0) return true;

    // Якщо тебе немає в списку, але в тебе є сторіс
    if (myIndex === -1 && myStories?.length > 0) return true;

    return false;
  };
  const myGradient = useMemo(() => getRandomGradient(), []);
        ...
        <div className={styles["storiesbtn-place"]}>
          <div
            onClick={() => {
              if (hasMyStories) {
                openModal();
                const myIndex = uniqueUsers.findIndex(
                  (u) => u.user.id === user?.sub
                );

                // Якщо є в списку — відкриваємо за індексом
                if (myIndex !== -1) {
                  setCurrentUserIndex(myIndex);
                  setCurrentStoryGroup(groupedStories[user.sub]);
                } else {
                  // Якщо тебе немає серед visible stories, додаємо свої вручну
                  setCurrentUserIndex(-1);
                  setCurrentStoryGroup(groupedStories[user.sub]);
                }
            ...
            className={styles["stories-btn"]}
            style={{
              background: hasMyStories ? myGradient : "none",

              padding: hasMyStories ? 3 : 0,
              borderRadius: "50%",
              boxSizing: "border-box",
            }}
          >
